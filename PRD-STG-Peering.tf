...
resource "aws_vpc_peering_connection" "PRD_STG_Peering" {
  vpc_id        = aws_vpc.PRD_CUS_VPC.id   # VPC A ID
  peer_vpc_id   = aws_vpc.STG_VPC.id   # VPC B ID
...
resource "aws_route" "PRD_CUS_VPC_to_STG_VPC" {
  route_table_id         = aws_route_table.PRD_CUS_RT_PUB.id
  destination_cidr_block = aws_vpc.STG_VPC.cidr_block  # VPC B의 CIDR 블록
...
resource "aws_route" "STG_VPC_to_PRD_CUS_VPC" {
  route_table_id         = aws_route_table.STG_RT_PUB.id
  destination_cidr_block = aws_vpc.PRD_CUS_VPC.cidr_block  # VPC A의 CIDR 블록
  vpc_peering_connection_id = aws_vpc_peering_connection.PRD_STG_Peering.id
}
